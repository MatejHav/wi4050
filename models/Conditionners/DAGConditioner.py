...
#         print(type(hidden))
    ...
    def post_process(self, zero_threshold=None):
        if zero_threshold is None:
            zero_threshold = .1
            G = nx.from_numpy_array((self.soft_thresholded_A().data.clone().abs() > zero_threshold).float().detach().cpu().numpy(), create_using=nx.DiGraph)
...
#             print(f'{e.shape}, {hot_encoding.shape}')
            ...
            return self.embedding_net(e)

        if context is not None:
            ctx = context.unsqueeze(1).expand(-1, self.in_size, -1).reshape(x.shape[0] * self.in_size, -1)
    ...
    def update_dual_param(self):
        with torch.no_grad():
            ...
            if self.dag_const > 0. and lag_const > self.tol:
                ...
                G = nx.from_numpy_array(self.A.detach().cpu().numpy() ** 2, create_using=nx.DiGraph)
                try:
                    nx.find_cycle(G)
