...
class ConditionalNormalLogDensity_UC(nn.Module):
    """Bivariate normal log density with per-sample rho learned from context via a small MLP."""
    def __init__(self, cond_dim):
        super(ConditionalNormalLogDensity_UC, self).__init__()
...
def buildFCNormalizingFlow_UC(nb_steps, conditioner_type, conditioner_args, normalizer_type, normalizer_args,
                              cond_in=0):
    if cond_in:
        conditioner_args = {**conditioner_args, 'cond_in': cond_in}
    flow_steps = []
    for step in range(nb_steps):
        conditioner = conditioner_type(**conditioner_args)
        normalizer = normalizer_type(**normalizer_args)
...
def buildFCNormalizingFlow_UC_conditional(nb_steps, conditioner_type, conditioner_args,
                                          normalizer_type, normalizer_args, cond_dim, cond_in=0):
    """V3: flow with conditional copula — rho is a function of observed context."""
    if cond_in:
        conditioner_args = {**conditioner_args, 'cond_in': cond_in}
    flow_steps = []
    for step in range(nb_steps):
