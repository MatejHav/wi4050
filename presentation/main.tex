...
% ── 15. Piecewise Copula: Setting ────────────────────────────────────────────
\begin{frame}{Extension: Stratified Models}
  \textit{DGP with heterogeneous confounding; $\rho$ depends on observed $X$.}
  \[
  ...
  \]
  \medskip
  \textbf{Three estimators compared:}
  \begin{itemize}
    \item \textcolor{orange}{\textbf{fixed $\rho=0.15$}} — global average, homogeneous assumption
    \item \textcolor{violet}{\textbf{fixed $\rho=0$}} — no confounding assumed
    \item \textcolor{ForestGreen}{\textbf{split $\rho$ (baseline)}} — two separate models, one per half; combined ATE:
    \[
      \hat\tau = \tfrac{n_-}{n}\,\hat\tau_- + \tfrac{n_+}{n}\,\hat\tau_+
    \]
  \end{itemize}
\end{frame}

% ── 16. Piecewise Copula: Results ─────────────────────────────────────────────
\begin{frame}{Extension: Stratified Models — Results}
  \begin{center}
    \includegraphics[width=0.95\textwidth]{figures/ate_figure.png}
  \end{center}
\end{frame}
...
\begin{frame}{Conclusion}
  ...
  \begin{itemize}[<+->]
    ...
    \item Stratified models: fit one $\rho$-GNF per covariate stratum, combine ATEs.
    \item Handles heterogeneous confounding — at the cost of $K$ model fits.
    \item \alert{But $\rho_k$ per stratum must still be specified by the practitioner.}
  \end{itemize}
\end{frame}
